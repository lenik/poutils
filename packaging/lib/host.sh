...
#
# Environment:
#   POUTILS_SRCDIR       project root (default: packaging/../..)
#   POUTILS_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
POUTILS_SRCDIR=${POUTILS_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$POUTILS_SRCDIR/.config/poutils" "$HOME/.config/poutils"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${POUTILS_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${POUTILS_PACKAGING:-}")"
...
cmd_run() {
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $POUTILS_SRCDIR/.config/poutils/$k.build-host" >&2
        echo "    $HOME/.config/poutils/$k.build-host" >&2
    done
    return 2
}
