...
static void fprintln_indented(FILE *out, const char *s) {
    fputs("  ", out);
    fprintln(out, s);
}
...
static void print_design_purpose(FILE *out) {
    ...
    fprintln_indented(out, _("Efficiently apply many already-translated l10n messages into PO files."));
    fprintln_indented(out, _("poedit is usually used to supplement translations by providing only newly "
                             "added entries."));
    fprintln_indented(out, _("This tool is mainly designed for AI-assisted coding workflows."));
    fputc('\n', out);
    fprintln(out, _("AI usage requirements:"));
    fprintln_indented(out,
                      _("1) Translate multiple messages in advance (manually) before running poedit."));
    fprintln_indented(out, _("2) Use poedit for batch update to avoid one-by-one tool calls."));
    fprintln_indented(out, _("3) Do NOT use poedit to generate placeholders or copy msgid to msgstr."));
    fprintln_indented(out, _("Doing so defeats the purpose of this tool."));
    fprintln_indented(out, _("4) AI must first identify missing entries in current translations,"));
    fprintln_indented(out, _("translate them manually."));
    fprintln_indented(out, _("then apply to PO files with this tool in one batch."));
}
...
void usage(FILE *out) {
    ...
    fprintln_indented(out, _("langs<TAB>msgid<TAB>msgstr[<TAB>msgstr...]"));
    fprintln(out, _("-or-"));
    fprintln_indented(out, _("msgid<TAB>msgstr[<TAB>msgstr...]"));
    fputc('\n', out);
    fprintln_indented(out, _("langs: comma-separated language codes; one msgstr column per lang."));
    fprintln_indented(out,
                      _("langs conflict resolution: [langs<TAB>] > [--langs] > [filename] > [subdir]"));
    fputc('\n', out);
